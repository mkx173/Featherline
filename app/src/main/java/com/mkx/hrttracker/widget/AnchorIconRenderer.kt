...
// own shape (circle/squircle/teardrop), so the gradient background is full-bleed and the
// number stays inside the inner 66% safe zone. Pure given (anchor, today): no I/O, no
...
object AnchorIconRenderer {
    ...
    fun render(anchor: TrackedDate, today: LocalDate): Bitmap {
        ...
        // Label in front, white bold, auto-shrunk to fit the safe-zone width. No glyph:
        // every placement tried read as clutter under the launcher masks.
        ...
        paint.textSize = fitTextSize(paint, label, maxWidth = SAFE_BOX * 0.85f, startSize = 150f)
