...
Featherline 支持注射、贴剂、凝胶、口服、舌下五种给药途径的剂量记录；基于三室药代动力学模型，从你的用药历史推算雌二醇水平曲线；并以化验项目目录为基础追踪血检结果，支持规范单位与临床单位之间的自动换算。数据默认存储于本地加密数据库，不要求账户、不进行遥测；Play 版本可选择通过用户自己的 Google Drive 同步端到端加密快照。提供英语与简体中文界面。
...
> **提示：** 推荐使用 Play 商店或 F-Droid 版本。F-Droid/GitHub 侧载版不申请网络权限，因此不包含 Google Drive 同步，也无法自动检查更新；如需获取新的 GitHub 构建，请手动关注发布页。
...
Featherline 将数据存储在本地加密数据库中，不要求 Featherline 账户，也不进行遥测。Play 版本可选择仅把端到端加密快照发送到用户私有的 Google Drive 应用数据目录。完整的数据处理说明请参见 [docs/privacy.md](docs/privacy.md)。
